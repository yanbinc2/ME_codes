...
1. Phase1. Run 'SpecClust.R' to generate the candidates seed regions. It outputs one file 'MNIST_Labels_Spec20.csv' in phase1/data/.
2. Phase2. Copy one output file 'MNIST_Labels_Spec20.csv' from phase1 and two orginal files 'MNIST_Labels_5000.csv' and 'MNIST_tSNE_5000.csv' into the phase2/data/. The hyperparameter file 'MNIST_generate_seedregions_params.txt' has been set and stored in the phase2/data/ already. Run 'generate_seedregions_package.m'. It will output three files.
3. Phase 3. Copy three output files 'MNIST_seedinds.txt', 'MNIST_bilabels.txt' and 'MNIST_seedinds_neighborregions.txt' from phase2 into the phase3/data/. Copy one output file 'MNIST_Labels_Spec20.csv' from phase1 into the phase3/data/. For the convenience, we converted 'MNIST_Labels_Spec20.csv' into pickle file 'Small_MNIST_tSNE_embeddings.pickle' and put it into phase3/data/. Run 'main_phase3.ipynb'. It will output several immediate pickle files and the users may ignore these pickle files. The three mat files 'results_of_original.mat', 'results_of_combination.mat' and 'results_of_removal.mat' will be generated to the phase3/codes/.
4. Phase 4. Copy three output files from phase3/codes/ into the phase4/data/. The hyperparameter file 'MNIST_merge_seedregions_params.txt' has been stored in phase4/data/. Run 'merge_seedregions_package.m'. It will output one file.
5. Phase 5. Copy one output file from phase4 into the phase5/data/. Run 'main_phase5.ipynb' to obtain the clustering results 'accu_history.csv' in the phase5/codes/.
